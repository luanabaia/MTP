...
int main()
{
    ...
    if(AUX > 0)
    {
        fseek(imagem, AUX, SEEK_CUR);//pule/escreva AUX bytes a partir do byte corrente (sem informação útil)
    }
    }
    fclose(imagem);
    ...
    // Copia cabeçalho
    imagem2 = fopen("novaimagem.bmp", "wb");
    rewind(imagem);
    for(i = 0; i < offset; i++)
    {
        fread(&byte, sizeof(byte), 1, imagem);
