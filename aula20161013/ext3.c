...
int main()
{
	...
	for(i = 0; i < npoligonos; i++)
        {
            printf("\nQuantos vertices tem o poligono %d? ", i+1);
            scanf("%d", &npontos);
...
Poligono recebePontos(int npontos)
{
	...
	printf("\nEntre com coordenadas separadas por espaço com vertices em sequencia anti-horaria ...\n");
	for(i = 0; i < npontos; i++)
        {
            printf("\nEntre com o %do ponto: ", i+1);
...
float calcArea(Poligono poligono)
{
	...
	for(i = 0; i < ultimo; i++)
		soma += poligono.vertices[i].x*poligono.vertices[i+1].y - poligono.vertices[i+1].x*poligono.vertices[i].y;
	soma += poligono.vertices[ultimo].x*poligono.vertices[0].y - poligono.vertices[0].x*poligono.vertices[ultimo].y;
	return 0.5*soma;
}
...
{
	int i;
	printf("\nQuantidade de vertices: %d; Vertices: ", P.nvertices);
